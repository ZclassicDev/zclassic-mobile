  ...
  const txPage = () => (
    <Page renderToolbar={() => (
      ...
      <List style={{wordBreak: 'break-word'}}>
        ...
        <ListItem tappable>
          <ons-row><strong>{ curTranslation.General.in }&nbsp;({tx.valueIn} ZCL)</strong></ons-row>
          {
            tx.vin.map(function (vin, idx) {
              return (
                <ons-row key={idx} style={{marginTop: '10px'}}>
                  <ons-col width={'90%'}>
                    { vin.addr }<br/>
                    <span style={{color: '#7f8c8d'}}>({ vin.value } ZCL)</span>
                  </ons-col>
        ...
        <ListItem tappable>
          <ons-row><strong>{ curTranslation.General.out } ({tx.valueOut} ZCL)</strong></ons-row>
          {
              ...
              return (
                <ons-row key={idx} style={{marginTop: '10px'}}>
                  <ons-col width={'90%'}>
                    ...
                    <span style={{color: '#7f8c8d'}}>({ vout.value } ZCL)</span>
                  </ons-col>
...
class MainPage extends React.Component {
  constructor (props) {
    ...
    this.state = {
      ...
      selectedAddressScannedTxs: false // Have we tried and found the txs? (used to display loading...)
    }
  ...
  setAddressInfo (address) {
    ...
    this.props.setZclInBtcValue(null)
    ...
    axios.get(addrURL)
      .then((resp) => {
        ...
        const cmcZclInfoURL = 'https://api.coinmarketcap.com/v1/ticker/zclassic/?convert=' + curCurrency
        axios.get(cmcZclInfoURL)
          .then((resp) => {
            try {
              ...
              this.props.setZclInBtcValue(priceBtc)
              this.props.setZclInCurrencyValue(priceCurrency)
            } catch (err) {
              if (err) {
                console.log(err)
              }
  ...
  componentDidMount () {
    ...
    window.ga.startTrackerWithId('UA-71060764-11', 30) //TODO Google Analytics ID
          ...
          <SplitterSide
            ...
            <Page>
              <List
                ...
                renderHeader={() => <ListHeader>ZCL</ListHeader>}
                renderRow={(i) =>
                  <ListItem
                ...
                <ons-col width={'47%'}>
                  <h1 style={{marginLeft: '12px'}}>
                    ...
                    }&nbsp;
                    {
                      this.props.context.value === null
                        ? null
                        : <span style={{fontSize: '16px'}}>ZCL</span>
                    }
                  </h1>
                </ons-col>
                <ons-col>
                          ...
                          <ListItem
                            ...
                            <ons-row>
                              ...
                              <ons-col style={{textAlign: 'right', paddingRight: '12px'}}>
                                { parseFloat(Math.abs(txValue)).toFixed(8) }&nbsp;ZCL
                              </ons-col>
                            </ons-row>
                          </ListItem>
                        )
...
MainPage.propTypes = {
  ...
  setZclInBtcValue: PropTypes.func.isRequired,
  setZclInCurrencyValue: PropTypes.func.isRequired
}
